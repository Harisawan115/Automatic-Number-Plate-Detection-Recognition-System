...
def load_yolo():
    ...
    return m, "⚠️ Base YOLOv8n — Train first for better accuracy"
...
def keep_best_plate_box(boxes):
    """
    FIX 1 — Multiple detections problem.
    Agar multiple boxes hain (jaise screenshot mein 3 thi),
    sirf sabse confident aur sabse plate-like (wide) box rakho.
    """
    if len(boxes) <= 1:
        return boxes
...
def preprocess_for_ocr(crop_bgr):
    """
    FIX 2 — Better preprocessing pipeline.
    EasyOCR works best on original color OR clean grayscale — NOT heavy threshold.
    Returns: (display_thresh, ocr_ready_image)
    """
...
def run_ocr(reader, crop_bgr, thresh):
    """
    FIX 3 — Multi-attempt OCR strategy.
    Try original color first, then grayscale, then threshold.
    Pick best result across all attempts.
    """
    candidates = []
